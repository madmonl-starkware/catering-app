...
const dummyUserData = {
  ...
  following: 42,
  freeches: [
    { id: "1", content: "Just deployed my first smart contract!", timestamp: "2 hours ago", isFeatured: false, viewCount: 120, likeCount: 15, isLiked: false },
    { id: "2", content: "Thoughts on the latest DeFi trends?", timestamp: "1 day ago", isFeatured: true, viewCount: 500, likeCount: 42, isLiked: true },
    { id: "3", content: "Working on a new blockchain project. Stay tuned!", timestamp: "3 days ago", isFeatured: false, viewCount: 300, likeCount: 28, isLiked: false },
  ]
}
...
export const FreechApp = () => {
  ...
  useEffect(() => {
    const userFreechTopicsLocalStorage = JSON.parse(localStorage.getItem('user_freech_topics')!);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        {showTopicSelection ? (
          <TopicSelection selectedTopicsA={selectedTopics} onComplete={handleTopicSelectionComplete} />
        ) : showUserProfile ? (
          <>
            <Button 
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <FreechFeed 
              loading={loading} 
              freeches={(freeches as IRawFreech[])?.map(({data, echo, id, preacher_id, time}) => ({
                author: preacher_id.toString(),
                content: data,
                timestamp: time as any,
                isFeatured: false,
                isLiked: Number(echo) > 0,
                likeCount: Number(echo),
