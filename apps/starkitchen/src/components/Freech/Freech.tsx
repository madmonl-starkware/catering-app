  ...
  return (
    <Card className={`mb-4 ${isFeatured ? 'border-primary' : ''}`}>
      ...
      <CardContent>
        ...
        {isFeatured && marketingBudget && (
          <div className="mt-4">
            ...
            <Progress value={(marketingBudget.used / marketingBudget.total) * 100} className="h-2" />
      ...
      <CardFooter className="flex justify-between">
        ...
        >
          <Heart className={`mr-2 h-4 w-4 ${localIsLiked ? "fill-current" : ""}`} />
          {localLikeCount}
        </Button>
          ...
          <Dialog open={isFeatureDialogOpen} onOpenChange={setIsFeatureDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Enter the amount of STRK tokens you want to spend to feature this post.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                ...
                {featureAmount && !isNaN(parseFloat(featureAmount)) && (
                  <div className="text-sm text-muted-foreground">
                    <p>Approx. ${getApproxUSD(parseFloat(featureAmount))} USD</p>
                    <p>Estimated {getApproxViews(parseFloat(featureAmount))} post views</p>
                  </div>
                )}
              </div>
              <DialogFooter>
